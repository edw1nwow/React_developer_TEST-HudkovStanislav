...
import {GET_ALL_CATEGORY_AND_CURRENCY} from '../../query/category'
import { NavLink } from 'react-router-dom'
import {useCurrentCurrency} from "../../helper/helper";



const Header = (props) => {
	const {loading, error, data} = useQuery(GET_ALL_CATEGORY_AND_CURRENCY)
	if (loading) return null;
	if (error) return `Error! ${error}`;


	const onCurrency = (el) => {
		let setCcyValue = el.currentTarget.value
		props.onChangeCurrency(setCcyValue)
	...
	return (
		<div className='header'>
			...
			<div className='header_rightside'>
				<div>
					<select onChange={onCurrency}  name="" id="">
						{data.currencies.map(el=>(
							<option key={el.symbol} value={el.symbol}>{el.symbol} </option>
						))}
					</select>
